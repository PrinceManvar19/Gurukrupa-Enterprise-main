...
const heroStats = [
  ...
  { value: 100, suffix: '%', label: 'SWAS delivery' },
]
...
export function HeroSection() {
  ...
  return (
    <section
      id="hero"
      className="hero-section relative flex flex-col justify-center overflow-hidden"
    >
      ...
      <div className="hero-inner container relative z-10 mx-auto grid items-center">
        <div className="hero-copy">
          <motion.div
            initial={{ opacity: 0, y: 14 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.65, ease }}
            className="hero-eyebrow inline-flex items-center gap-2 rounded-full border border-accent/20 bg-card/70 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-accent backdrop-blur"
          >
            <Workflow className="h-4 w-4" />
            Software WITH a Service
          </motion.div>

          <h1 className="hero-title max-w-5xl font-semibold text-foreground">
          ...
          <motion.p
            ...
            className="hero-subtext text-muted-foreground"
          >
            Built in Ahmedabad and deployed across India. Gurukrupa Enterprise builds software products,
            custom web platforms, mobile apps, AI automation, and operational systems through SWAS:
            Software WITH a Service.
          </motion.p>
          ...
          >
            {heroStats.map((stat) => (
              <div key={stat.label} className="stat-box rounded-2xl border border-accent/15 bg-card/60 backdrop-blur">
                <div className="stat-num font-semibold text-foreground">
                  <StatCounter value={stat.value} suffix={stat.suffix} />
                </div>
                <div className="stat-label font-medium uppercase tracking-widest text-muted-foreground">{stat.label}</div>
              </div>
            ))}
          </motion.div>
          ...
          <motion.div
            ...
            className="hero-btns flex flex-col sm:flex-row"
          >
            <motion.a
              href="/contact"
              className="inline-flex items-center justify-center rounded-lg btn-premium px-6 py-3 text-sm font-semibold text-primary-foreground"
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
            >
              Start a Project
            ...
            <motion.a
              href="#swas"
              className="inline-flex items-center justify-center rounded-lg border border-border bg-card/70 px-6 py-3 text-sm font-semibold text-foreground backdrop-blur transition hover:border-accent/35 hover:bg-accent/10"
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
            >
              Explore SWAS
        ...
        <motion.div
          ...
          className="relative"
        >
          <div className="hero-swas-card glass-card relative overflow-hidden rounded-2xl">
            ...
            <div className="mt-4 grid gap-2 sm:grid-cols-3">
              {['Hands-on Support', 'Workflow Automation', 'Continuous Optimization'].map((item, index) => (
                <motion.div
                  ...
                  className="rounded-xl border border-border bg-background/55 px-3 py-2 text-xs font-medium text-foreground"
                >
                  {item}
                </motion.div>
              ))}
